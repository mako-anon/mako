...
class LabelGenerator:
    ...
    def generate_hyperparams(self, idx=None, bad_configs=[]):
        ...
        if self.task in ['mnist', 'fashion']:
            ...
            n_epochs_search_space = [15, 20, 25]
        elif self.task in ['cifar10']:
            lr_search_space = [1e-2, 0.8e-3, 1e-3]
            n_batches_search_space = [20]
            n_epochs_search_space = [30, 40, 50, 60]
        else:
            raise NotImplementedError
                ...
                for n_epochs in n_epochs_search_space:
                    for lr in lr_search_space:
                        ...
                        if self.task in ['mnist', 'fashion']:
                            ...
                            model = OmniglotWeakLabeler(in_dim_c=X.shape[1], in_dim_h=X.shape[2],
                                                 in_dim_w=X.shape[3], out_dim=self.num_classes,
                                                 dict_training_param=dict_training_params).to(DEVICE)
                        elif self.task in ['cifar10']:
    ...
    def generate_snuba_lfs(self, log_file):
        ...
        with open(log_file, 'a') as f:
            ...
            while len(hg.hf) < self.max_labelers:
                ...
                # Confidence exit condition: Stop the iterative process when no low confidence labels
                # Also exit when only 1 data point is unconfident since we can't use batch normalization
                if len(new_idx) == 0 or len(new_idx) == 1:
                    f.write("Exit: new_idx is empty\n")
                    break
